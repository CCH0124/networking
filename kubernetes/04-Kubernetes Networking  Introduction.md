...
Kubernetes 網路模型的核心設計旨在提供原生的多主機集群網路，並依賴 Container Network Interface (CNI) 專案來滿足嚴格的網路要求：

1. **Pod 間通訊無 NAT (No NAT for Pod-to-Pod):** 集群中的所有 Pod（無論位於哪個節點）必須能夠在不使用網路地址轉換（NAT）的情況下直接互相通訊。
2. **節點與 Pod 間通訊無 NAT:** 節點必須能夠在不使用 NAT 的情況下與所有 Pod 通訊。
3. **IP 一致性:** 容器內部看到的 IP 地址必須與集群外部看到的一致。

* **Pod 作為原子單位:** Kubernetes 的基本工作單元是 Pod，它包含一或多個共享網路命名空間的容器。這確保了在同一 Pod 內，容器之間的通訊就像在同一主機上運行一樣。
* **Pod IP 的性質 (Ephemeral):** 每個 Pod 擁有一個唯一的IP地址，但這個地址是短暫的（ephemeral）。這使開發者無需處理埠衝突，但同時也意味著客戶端不能依賴這些 Pod IP 進行手動配置或 DNS 記錄。
* **Kubelet 與 CNI 的協同:** Kubelet 在節點上負責管理 Pod，並透過 CNI 插件進行網路配置和 IP 地址分配。Kubernetes 標準安裝並不包含預設的 CNI 插件。
...
Pod 和節點之間必須具備 L3 層的連通性。CNI的選擇和實作將決定集群的網路佈局：

* **Isolated Networks (隔離網路):** 節點在廣域網路上可路由，但 Pod IP 在集群外部不可路由。多個集群可以使用相同的 IP 地址空間。
* **Flat Networks (平面網路):** 所有 Pod IP 地址在廣域網路上都是可路由的。優點是網路架構簡單、延遲低，但要求為集群分配一個非常大的連續 IP 地址空間。
* **Island Networks (島嶼網路):** 節點可路由，Pod 不可路由。Pod 的出站流量通常會通過**SNAT (Source NAT)** 進行偽裝（masquerading），使流量看起來來自節點的IP地址。
...
CNI 還負責調用 IPAM 專門負責 IP 地址的分配、閘道和路由資訊的確定。
